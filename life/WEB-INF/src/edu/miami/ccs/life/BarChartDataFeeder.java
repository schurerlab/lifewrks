...
public class BarChartDataFeeder extends HttpServlet {
	/**
	...
	protected void doGet(HttpServletRequest request,
		...
		if (!mode.equals("CellLine")) {
			...
			String BarQuery = "select count(e2p2.participantid) as ActiveCompoundCount,p.name as Pname,p.id as Pid,p.paticipanttype as Ptype"
					+ "  from endpoint e "
			...
			String BarQuery = "select count( e2p.participantid) as ActiveCompoundCount,p.name as Pname,p.id as Pid,p.paticipanttype as Ptype" +
					" from endpoint e "
					+ " inner join endpoint_has_participant e2p on (e2p.endpointid = e.id) "
					+ " inner join endpoint_has_participant e2p2 on (e2p2.endpointid = e.id) "
					+ "  inner join participant p on (p.id = e2p2.participantid) "
