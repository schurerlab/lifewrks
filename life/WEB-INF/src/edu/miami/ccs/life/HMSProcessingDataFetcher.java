...
public class HMSProcessingDataFetcher extends HttpServlet {
	/**
  ...
  if (!input.equals(null)){
		...
		String json = "";
		json += "{ \"results\": [";
		String Query = "select distinct ep.name as Type,count(distinct pm.name) as countOf " +
				"from endpoint e"
		...
		String QueryForData = "select distinct ep.name as Perturbation_Type,pm.name  as Perturbation_Measure, count(pm.name) as count from endpoint e"
				+ " inner join epname ep on (ep.epid = e.epname_epid)"
				+ "   inner join perturbation_measure pm on (pm.measureid = e.measureid)"
				+ "      inner join endpoint_has_participant ehp on  (ehp.endpointid = e.id)"
				+ "        INNER JOIN endpoint_has_participant ehp2 on (ehp2.endpointid = e.id)"
