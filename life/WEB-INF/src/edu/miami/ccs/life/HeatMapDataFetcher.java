...
public class HeatMapDataFetcher extends HttpServlet {
	/**
	...
	protected void doGet(HttpServletRequest request,
		...
		String QueryForData = "select distinct p2.name as ParticipantName,e.value as value,e.timepoint as Time," +
				" e.concentration as Concentration,e.id as eid"
				+ " from endpoint e"
				...
				+ "       and ep.name ='"
				+ type.trim()
				+ "' and pm.name ='"+ measure.trim() +"' ";
				if (timepoint!=null){
					QueryForData+=" and e.timepoint="+timepoint.trim() 
				+" and e.concentration ="+concentration.trim() + " and e.epname_epid ="+epid.trim();
				}
				QueryForData+=" order by e.value desc";
		...
		SQLQuery query = ses.createSQLQuery(QueryForData)
				...
				.addScalar("eid", Hibernate.INTEGER);
		...
		if (objects.hasNext()) {
			...
			while (objects.hasNext()) {
				...
				json += "{" +"\"eid\":\"" + tuple[4].toString() + "\","
						+"\"participant\":\"" + tuple[0].toString() + "\","
