$(document).ready(function() {
	...
	$('#content').transition({ x: '0', y: '-50px'}, 3500, 'cubic-bezier(0.9,0.9,0.3,1)');
	
	// show and hide tags container
	$("footer").click(function() {
    	var height= $('#tags-container').height(); 
		
		if(!$('footer').hasClass('clicked')){
    		
		$('footer').transition({ x: '0', y: -height}, 3500, 'cubic-bezier(0.9,0.9,0.3,1)');
    		$('footer').addClass('clicked');
    		$('#tags-header').html('Hide Tags');
    		}
    		else{
    		$('footer').transition({ x: '0', y: '0'}, 3500, 'cubic-bezier(0.9,0.9,0.3,1)');
			...
			function loadProteins()
			{
				...
				isSorted = "false";
				$.ajax( {url: "http://lincs.ccs.miami.edu:8080/solr-example/select", data: $("#searchForm").serialize(), success: function(response) {
					...
					{		
						if(groups[i].groupValue != null)
						{
							...
							tableData += '['
									...
									+'","<a href = \'#\' title = \'Participant Counts\' rel=\'/solr-example/select?q='+assayCountTerm+'&group=true&group.field=SmallMoleculeId&group.field=ProteinId&group.field=GeneId&group.field=CellLineId&wt=json&indent=true&rows=-1\' class = \'participantCount\'>View</a>"]';
