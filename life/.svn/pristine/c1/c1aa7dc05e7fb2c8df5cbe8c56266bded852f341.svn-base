...
public class CompareTableFeeder extends HttpServlet {
	/**
		...
		if (!mode.equals("CellLine")) {
			...
			for (i = 0; i < clist.length; i++) {
				...
				String DataQuery = "select distinct p.name as Pname,p2.name as CName,e.value as Value "
						...
						+ " where e2p.participantid =" +clist[i]
						+ " and e2p2.participantid in ("+targetidlist+" ) "
						+ " and e.epname_epid ="
						+ epid
						+ " and e.timepoint ="
						+ timepoint + "and e.concentration =" + concentration;
				...
				if (dataobjects.hasNext()) {
					...
					while (dataobjects.hasNext()) {
						...
						json += "{ \"xEle\":" + "\"" + datalist[1].toString()
								+ "\"," + "\"yEle\":" + "\""
								+ datalist[0].toString() + "\"," + "\"value\":"
								+ "\"" + datalist[2].toString() + "\"" + "}";
