...
public class AssayView extends HttpServlet {
	/**
	...
	protected void doGet(HttpServletRequest request,
		...
		SolrQuery query = new SolrQuery(input);
		 query.set("facet",true );
		  ...
		  query.addFacetField("assayName");
		  query.set("rows", 0);
		  query.set("wt", "json");
		  query.set("indent", true);
		  query.set("json.nl", "map");
		  query.set("q.op","AND");
		  QueryResponse assayResponse = null;
	
		  try {
				
			  assayResponse = server.query(query);
			} catch (SolrServerException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		    List<FacetField> facets = assayResponse.getFacetFields();
			...
			String assays = assayResponse.getFacetFields().get(0).getValues().get(x).getName();
				
		server = new CommonsHttpSolrServer(url);
		query = new SolrQuery(input + " "+assays);
		  query.set("facet",true );
		  query.set("facet.limit",-1 );
		  query.set("facet.mincount",minCount );
