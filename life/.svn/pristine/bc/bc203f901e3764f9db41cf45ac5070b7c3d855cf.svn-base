 ...
 * 
 */
public class FetchViewDataCellBased extends HttpServlet {
	String url = "";
	...
	String parCoordStr = "";
	String searchTerm = "", diseasename = "", assayTypeName = "";
	...
	protected void doGet(HttpServletRequest request,
		...
		assayTypeName = request.getParameter("assayTypeName");

		searchTerm = URLEncoder.encode(request.getParameter("q") + " Disease:"
				+ '"' + diseasename + '"' + " AssayTypeName:" + '"' + assayTypeName
				+ '"', "UTF-8");
		...
		try {
			...
			cellLineFacetObject = cellLineNameJsonObj.getJSONObject(
					"facet_counts").getJSONObject("facet_fields");
			...
			// get signature ids
			signatureIdJsonObj = getSignatureIdData(searchTerm, cellLinestr,
					lsmStr);

			signatureIds = signatureIdJsonObj.getJSONObject("grouped")
					.getJSONObject("PerturbationId");
	...
	private JSONObject getCellLineHeaders(String q) throws SolrServerException,
		...
		url += "&facet=true&facet.field=LincsSMId&facet.field=CellLineName&facet.mincount=1&facet.limit=-1";
	...
	private JSONObject getSignatureIdData(String q, String cellLines,
		...
		url = "http://baoquery.ccs.miami.edu/cellbased-solr/select?";
		url += "&wt=json&rows=-1";
		...
		try {
			...
			URL obj = new URL(url);
			HttpURLConnection urlConn = (HttpURLConnection) obj
					.openConnection();

			String urlParameters = "q=" + lsmIds + "+AssayTypeName:" + '"'
					+ assayTypeName + '"';
			urlParameters += "&fl=LincsSMId,TimePoint,SMCategories,Concentration,PerturbationType,"
					+ cellLines;
			...
			DataOutputStream wr = new DataOutputStream(urlConn
					.getOutputStream());
	...
	private String getDataForParCoord(JSONArray signatureIdData,
			JSONArray cellLineName) throws IOException {
